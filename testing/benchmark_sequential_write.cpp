/**
 * Benchmark: Sequential Write Performance Test
 * 
 * Purpose: Measure the write performance of the distributed file system
 * with sequential write patterns to stress test the page cache.
 * 
 * Test Scenario:
 * - Creates multiple large files and writes data sequentially
 * - Measures throughput, latency, and ops/sec
 * - Designed to test page cache write-back behavior
 * 
 * Setup: 
...
public:
    SequentialWriteBenchmark(const BenchmarkConfig& config) : config_(config) {
        auto channel = grpc::CreateChannel(
            config_.master_addr,
            grpc::InsecureChannelCredentials()
        );
        stub_ = FSMasterService::NewStub(channel);
        stats_.test_name = "Sequential Write";
    }
    ...
    void RunBenchmark() {
        std::cout << "\n" << BLUE << "================================" << RESET << std::endl;
        std::cout << BLUE << "Sequential Write Benchmark" << RESET << std::endl;
        ...
        std::cout << "Total Data: " << (config_.num_files * config_.file_size_kb / 1024.0) 
                  << " MB" << std::endl;
        ...
        for (uint64_t file_idx = 0; file_idx < config_.num_files; ++file_idx) {
            std::string filepath = "/benchmark/seq_write_" + std::to_string(file_idx) + ".dat";
            ...
            // Write file sequentially in chunks
            for (uint64_t chunk_idx = 0; chunk_idx < chunks_per_file; ++chunk_idx) {
                ...
                if (success) {
                    ...
                    stats_.failed_ops++;
                    if (config_.verbose) {
                        std::cerr << RED << "Write failed at offset " << offset << RESET << std::endl;
                    }
                }
            }
            ...
            // Progress indicator
            if ((file_idx + 1) % 10 == 0 || file_idx == config_.num_files - 1) {
                std::cout << "\rProgress: " << (file_idx + 1) << "/" << config_.num_files 
                          << " files completed" << std::flush;
            }
        }
        std::cout << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << CYAN << "║     Sequential Write Benchmark - Page Cache Performance      ║" << RESET << std::endl;
    std::cout << CYAN << "╚══════════════════════════════════════════════════════════════╝" << RESET << std::endl;

    SequentialWriteBenchmark benchmark(config);
